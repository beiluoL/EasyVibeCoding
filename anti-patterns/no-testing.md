...
- "手动点几下就能看出对错，写测试太慢"——感觉测试是额外成本。
- MVP 阶段功能少，手动验证确实能覆盖，让人误以为规模大了也行。
- "AI 说做完了，我试了一下能用"——看起来验证过了，其实是抽查。
...
- 每个功能至少配 1 条验收测试（能自动判定通过/失败）。
- 改代码后必须跑测试套件，不靠"我再手动点一遍"。
- AI 说"做完了"时，第一句问：测试在哪？跑过了吗？
